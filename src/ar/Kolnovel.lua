...
return {
        ...
        Listing("Novel List",true,function (data)
            ...
            Listing("Latest",true,function (data)
                ...
                return map(d:select("#content  div  div.postbody  div  div.latesthome~div.listupd  div.excstf  div.utao  div.uta"),function (v)
                        local nov = Novel()
                        nov:setImageURL(d:select("div.imgu  a  img"):attr("src"))
                        nov:setTitle(d:select(" div.luf  a  h3"):text())
                        nov:setLink(d:select(" div.luf  a.series"):attr("href"))
                        return Novel {
    ...
    parseNovel = function (novelURL)
        ...
        novelInfo:setAuthors(map(document:select("div.bixbox.animefull > div.bigcontent > div.infox > div > div.info-content > div.spe > span:nth-child(3)  a"),function (v)
            return v:text()
        end))

        novelInfo:setGenres(map(document:select("div.bixbox.animefull > div.bigcontent > div.infox > div > div.info-content > div.genxed a"),function (v)
            return v:text()
        end))
